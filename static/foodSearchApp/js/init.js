...
function load_more_search_data() {
    ...
    if ((user_latitude !="") && (user_longitude !="") && (!loading_data_flag)){
      ...
      // xhttp.ontimeout = function () {document.getElementsByClassName('search-results-loader')[0].style.display = 'none';}
      xhttp.onreadystatechange = function() {
          if (this.readyState == 4 && this.status == 200) {
              response = JSON.parse(this.responseText);
...
function render_search_results(food_detail_data) {
  for (var i = 0; i < food_detail_data.length; i++) {
    ...
    //search_results_elmt = document.getElementsByClassName('search-results')[0]
    //search_results_elmt.scrollTop = search_results_elmt.scrollHeight-100;
